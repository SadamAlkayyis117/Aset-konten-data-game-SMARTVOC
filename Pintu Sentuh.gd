...
# Asumsi TouchScreenUI adalah Autoload/Global Singleton
@onready var touchscreen_ui = get_node("/root/TouchScreenUI") 
...
func _ready():
    ...
    # 🌟 KOREKSI: Pastikan TouchScreenUI valid sebelum menghubungkan 🌟
    if is_instance_valid(touchscreen_ui) and touchscreen_ui.has_signal("ui_interact_pressed"):
        touchscreen_ui.ui_interact_pressed.connect(self.on_ui_interact)
        
    collider_geser.set_deferred("disabled", false) 
    
    # KOREKSI PENTING: Panggil hide_interaction_prompt() saat ready untuk cleanup awal
